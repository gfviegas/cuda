...
__global__ void sumArraysOnGpu(float *A, float *B, float *C, int* I, int* R, int strike, float *glocal, const int N) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i+4 < N) {
        for (int j=0; j < 4; j++) 
            glocal[i+j] = 2*A[i+j];
        C[i] = A[i] + B[i] + glocal[i+threadIdx.x%4];
    }
}
...
int main(int argc, char**argv) {
  ...
  float *h_A, *h_B, *hostRef, *gpuRef, *glocal;
  int *R, *I;
  ...
  sumArraysOnGpu<<<grid, block>>>(d_A, d_B, d_C, I, R, strike, glocal, nElem);
